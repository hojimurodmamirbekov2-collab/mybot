...
def add_admin_btn(msg):
    ...
    safe_send(msg.chat.id,
        ...
        "💡 Admin o'z ID sini bilish uchun botga <code>/myid</code> yozsin\n\n"
...
def remove_admin_callback(call):
    ...
    if result:
        bot.answer_callback_query(call.id, f"✅ Admin o'chirildi: {target_id}", show_alert=True)
        try:
        ...
        bot.answer_callback_query(call.id, "❌ Admin topilmadi", show_alert=True)
...
def admin_steps(msg):
    ...
    if step == "code":
        ...
        try:
            ...
            safe_send(msg.chat.id, "❗ ID faqat raqam bo'lishi kerak!\nMisol: <code>123456789</code>", parse_mode="HTML")
            del admin_states[msg.from_user.id]
            return
        ...
        try:
            safe_send(new_id, "🎉 Tabriklaymiz! Siz endi botning admin'isiz.\n\n/start bosing — admin paneli ochiladi.")
        except:
            pass
        del admin_states[msg.from_user.id]
